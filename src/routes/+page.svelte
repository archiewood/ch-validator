<script lang="ts">
    ...
    let validationResult = $derived(
        useSchema
            ? validateWithSchema(sql, schema, "clickhouse", {
                  checkTypes: true,
                  semantic: true,
                  checkReferences: true,
              })
            : validate(sql, "clickhouse", {
                  semantic: true,
                  checkReferences: true,
              }),
    );

    let highlighter = $state<Awaited<ReturnType<typeof getHighlighter>> | null>(
        null,
    );
...
<svelte:head>
    ...
    />
    <link
        href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;700&family=JetBrains+Mono:wght@400;500;600&display=swap"
        rel="stylesheet"
    />
</svelte:head>
    ...
    <main>
        ...
        >
            <div class="editor-chrome">
                ...
                <label
                    class="schema-toggle"
                    title="When enabled, validates SQL against the defined table schema. When disabled, only checks SQL syntax."
                >
            ...
            <div class="editor-area">
                ...
                >
                    ...
                    {:else}
                        <pre class="highlight-placeholder"><code
                                >{sql || " "}</code
                            ></pre>
                    {/if}
                </div>
                ...
                <ul>
                    {#each validationResult.errors as error, i}
                        <li style="animation-delay: {i * 40}ms">
                            ...
                            {#if error.line != null}
                                <span class="location">
                                    Ln {error.line}{#if error.column != null},
                                        Col {error.column}{/if}
                                </span>
                            {/if}
                        </li>
        ...
        <footer>
            <span
                >powered by <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer">polyglot</a
                >
            </span>
        </footer>
    </main>
...
<style>
    ...
    .editor-container.is-valid:focus-within {
        ...
        box-shadow:
            0 0 0 1px color-mix(in srgb, var(--green) 8%, transparent),
    ...
    .editor-container.is-invalid:focus-within {
        ...
        box-shadow:
            0 0 0 1px color-mix(in srgb, var(--red) 6%, transparent),
